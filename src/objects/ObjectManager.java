...
public class ObjectManager {
	...
	public void update(int[][] lvlData, Player player, Player2 player2) {
		...
		updateCannons(lvlData, player, player2);
		updateProjectiles(lvlData, player,player2);
	...
	private void updateProjectiles(int[][] lvlData, Player player, Player2 player2) {
		for (Projectile p : projectiles)
			if (p.isActive()) {
				p.updatePos();
				if ((p.getHitbox().intersects(player.getHitbox())) && (player.getState()!=6)) {
					player.changeHealth(-25);
					p.setActive(false);
				} else if (IsProjectileHittingLevel(p, lvlData))
					p.setActive(false);
				if ((p.getHitbox().intersects(player2.getHitbox())) && (player2.getState()!=6)){
					player2.changeHealth(-25);
					p.setActive(false);
				} else if (IsProjectileHittingLevel(p, lvlData))
					p.setActive(false);
			}
	...
	private void updateCannons(int[][] lvlData, Player player,Player2 player2) {
		for (Cannon c : currentLevel.getCannons()) {
			if (!c.doAnimation)
				if (c.getTileY() == player.getTileY())
					if (isPlayerInRange(c, player))
						if (isPlayerInfrontOfCannon(c, player))
							if ((CanCannonSeePlayer(lvlData, player.getHitbox(), c.getHitbox(), c.getTileY()))&&(player.getState()!=6))
								c.setAnimation(true);
			if (!c.doAnimation)
				if (c.getTileY() == player2.getTileY())
					if (isPlayerInRange(c, player2))
						if (isPlayerInfrontOfCannon(c, player2))
							if ( (CanCannonSeePlayer(lvlData, player2.getHitbox(), c.getHitbox(), c.getTileY()))&&(player2.getState()!=6)   )
								c.setAnimation(true);
